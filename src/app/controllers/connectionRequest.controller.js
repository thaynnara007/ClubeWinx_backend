...
const requestConnection = async (req, res) => {
  try {
    ...
    log.info(`Inicializando criação da requisição de conexão. sendedId=${user.id}, requestedUser=${userId}`)
    ...
    if (connection)
      return res
        ...
        .json({ error: "Esses usuários já são conectados ou já existe uma requisição de conexão entre ambos" })
    
    log.info(`Criando requisição de conexão no banco de dados. requestedUserId=${userId}, sendedUserId=${user.id}`)    
    ...
    return res.status(StatusCodes.CREATED).json("Requisição de conexão feita com sucesso")
  } catch (error) {
